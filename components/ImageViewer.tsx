...
export const ImageViewer: React.FC<ImageViewerProps> = ({ isOpen, asset, allAssets, onClose, onUpdateAsset, onDeleteAsset, onMagicEdit, onSelectAsset, isSelected }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-lg flex flex-col items-center justify-center z-[60]" onClick={handleClose}>
            ...
            <div className="flex flex-col w-full h-full pt-16 pb-4 px-4 gap-4 max-w-7xl mx-auto" onClick={e => e.stopPropagation()}>
                <main className="flex-1 flex items-center justify-center relative overflow-hidden">
                    <div className="relative group/viewer h-full w-full flex items-center justify-center">
                        <img src={imageUrl} alt={currentAsset.name} className="max-w-full max-h-full w-auto h-auto object-contain rounded-lg" />
                ...
                <footer className="flex-shrink-0 text-left">
                    <div className="space-y-2 text-sm max-w-3xl">
                        ...
                        >
                            <Icon 
                                ...
                                className={`w-5 h-5 text-slate-400 transition-transform duration-200 flex-shrink-0 ${isDetailsExpanded ? 'rotate-0' : '-rotate-90'}`}
                            />
                            {isEditingName ? (
                                <input 
                                    type="text" 
                                    ...
                                    autoFocus 
                                    className="bg-slate-900 text-lg font-bold w-auto max-w-full p-1 rounded border border-cyan-500 text-left flex-1" 
                                />
                            ) : (
                                <h2 
                                    className="text-lg font-bold text-white break-words flex-1" 
                                    onDoubleClick={(e) => {
                                        e.stopPropagation();
